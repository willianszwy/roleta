...
interface PrizeRouletteProps {
  ...
  selectedPrize?: Prize;
  onSpin: () => Promise<{ participant: Participant | null, prize: Prize | null }>;
  onSpinComplete: (participant?: Participant, prize?: Prize) => void;
}
...
const PrizeText = styled.text<{ $textColor: string; $fontSize: number }>`
...
export const PrizeRoulette: React.FC<PrizeRouletteProps> = ({
  participants,
  prizes,
  isSpinning,
  selectedParticipant,
  ...
  const handleSpin = async () => {
    if (participants.length === 0 || prizes.length === 0 || isSpinning) return;

    const result = await onSpin();
    
    if (result.participant && result.prize) {
      ...
      const rotationToAdd = calculateRouletteRotation(prizeIndex, prizes.length, rotation, extraRotations);
      const newRotation = rotation + rotationToAdd;
      setRotation(newRotation);
      
      setTimeout(() => {
        onSpinComplete(result.participant!, result.prize!);
      }, duration * 1000);
    }
  };
        ...
        <RouletteWrapper>
          <EmptyWheel size={wheelSize}>
            {participants.length === 0 && prizes.length === 0 
              ? 'Adicione participantes e prêmios para começar'
              : participants.length === 0 
                ? 'Adicione participantes para começar'
                : 'Adicione prêmios para começar'
            }
          </EmptyWheel>
        </RouletteWrapper>
      </RouletteContainer>
  ...
  // Calculate appropriate font size based on wheel size and number of prizes
  const fontSize = Math.max(8, Math.min(16, (wheelSize * 0.8) / prizes.length));

  return (
    <RouletteContainer>
      <StatusMessage>
        {prizes.length} prêmio{prizes.length > 1 ? 's' : ''} disponível{prizes.length > 1 ? 'eis' : ''} para {participants.length} participante{participants.length > 1 ? 's' : ''}
      </StatusMessage>
        ...
        <WheelContainer size={wheelSize}>
          ...
          >
            ...
            {prizes.map((prize, index) => {
              ...
              // Get color for this prize
              const prizeColor = prize.color || colors[index % colors.length];
              ...
              return (
                <g key={prize.id}>
                  ...
                  <PrizeText
                    x={textX}
                    y={textY}
                    $textColor={getContrastColor(prizeColor)}
                    $fontSize={fontSize}
      ...
      <SpinButtonContainer>
        <SpinButton
          disabled={isSpinning || participants.length === 0 || prizes.length === 0}
          onClick={handleSpin}
          whileHover={!(isSpinning || participants.length === 0 || prizes.length === 0) ? { scale: 1.02 } : {}}
          whileTap={!(isSpinning || participants.length === 0 || prizes.length === 0) ? { scale: 0.98 } : {}}
        >
      ...
      <AnimatePresence>
        {selectedParticipant && selectedPrize && !isSpinning && (
          <ResultDisplay
          ...
          >
            <WinnerText>🎉 Ganhador 🎉</WinnerText>
            <WinnerName>{selectedParticipant.name}</WinnerName>
            <WinnerText>ganhou</WinnerText>
            <PrizeName>🎁 {selectedPrize.name}</PrizeName>
          </ResultDisplay>
        )}
      </AnimatePresence>
    </RouletteContainer>
